...
## 📝 Estudo de Caso: Classificação de Transações Financeiras Mistas na Gig Economy (SaaS B2B2C)

O projeto aborda cientificamente a sobreposição de finanças pessoais e profissionais de trabalhadores autônomos. A proposta investiga como técnicas de Inteligência Artificial e Processamento de Linguagem Natural (NLP) podem automatizar a separação dessas contas de forma ética e fiduciária, inspirada na literatura de engenharia de software e gestão de incertezas (método científico Lean).

*   **Problema de Negócio:** Profissionais autônomos e freelancers misturam finanças pessoais e profissionais em uma única conta corrente, gerando ineficiência tributária, perda de tempo em conciliações manuais e restrições no acesso a linhas de crédito corporativas.
*   **Problema de IA:** Classificar descrições textuais curtas, cruas, poluídas e truncadas de extratos bancários (*bank feeds*) em perfis de gastos: **Pessoal**, **Profissional** ou **Misto**.

---

## 📂 Organização das Entregas (Atividade 01)
...
1.  **Fonte de Dados Realista:** [`transacoes_financeiras_anonimizadas.csv`](transacoes_financeiras_anonimizadas.csv)  
...
2.  **Relatório Técnico de Dados:** [`atividade-01-dados-v2.md`](docs/atividade-01-dados-v2.md) (também disponível como PDF formatado: [`proposta-atividade-01.pdf`](proposta-atividade-01.pdf))  
...
4.  **Visualização Estatística:** [`distribuicao_categorias.png`](distribuicao_categorias.png)  
    Gráfico gerado no script mostrando a distribuição volumétrica das transações e categorias de consumo em relação à variável-alvo.

---

## ⚙️ Roteiro de Execução e Reprodutibilidade

Para executar a análise exploratória básica e verificar de forma automatizada o volume e a qualidade das transações, certifique-se de ter instalado o `pandas`, `numpy` e `matplotlib`.

Rode o script em seu terminal a partir da raiz do repositório:

```bash
python 01_exploracao_dados.py
```
...
*   **Distribuição da Variável-Alvo (`tipo_conta_estimado`):**
    *   **Pessoal:** 3.485 transações (69,70%)
    *   **Misto:** 770 transações (15,40%)
    *   **Profissional:** 745 transações (14,90%)
...
1.  **Os dados necessários para o projeto estão efetivamente disponíveis?**  
    Sim. Dispomos de uma amostra estruturada em CSV com 5.100 registros reais anonimizados de transações financeiras via Open Finance. Essa base provê uma representação robusta e diversificada para treinar os algoritmos iniciais de processamento de texto (NLP) e classificação de gastos sem depender de APIs externas pagas para a validação do MVP acadêmico.
...
---
*Declaração de Integridade Acadêmica: Os dados utilizados nesta atividade são simulados de forma realista e anonimizados, não contendo dados pessoais, sigilosos ou sensíveis, em conformidade com as diretrizes de privacidade da UFBA.*
